...
function StatBox({ label, value, sub, icon: Icon, color = 'text-primary' }) {
  return (
    <div className="bg-dark-bg border border-dark-border rounded-2xl p-5">
      <div className="flex items-center gap-2 text-slate-500 text-xs uppercase tracking-wide font-semibold mb-3">
        {Icon && <Icon size={13} className={color} />}
        {label}
      </div>
      <div className="text-slate-900 text-3xl font-bold">{value}</div>
...
export default function Reports() {
  ...
  const countConfig   = { count:   { label: 'Trips',       color: '#16A34A' } };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-slate-900 text-2xl font-bold">Reports</h1>
          <p className="text-slate-500 text-sm">Revenue, trips and performance analysis</p>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            {downloading
              ...
              : <DownloadCircle01Icon size={14} />}
            Weekly Report
          </button>
          <button
            onClick={handleDownloadCurrent}
            disabled={!data}
            className="flex items-center gap-2 px-4 py-2 bg-dark-card border border-dark-border text-slate-600 rounded-xl text-sm font-medium hover:border-primary/50 hover:text-slate-800 transition-colors disabled:opacity-40"
          >
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-2">
          <span className="text-slate-500 text-xs font-medium">Quick:</span>
          <button
            onClick={setLastWeek}
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="bg-dark-card border border-dark-border rounded-2xl p-5">
            <div className="mb-5">
              <h2 className="text-slate-900 font-semibold text-lg">Daily Revenue</h2>
              <p className="text-slate-500 text-xs mt-0.5">{from} → {to}</p>
            </div>
            {data?.daily?.length > 0 ? (
              <ChartContainer config={revenueConfig} className="h-[260px]">
                <AreaChart data={data.daily} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                  ...
                  <Area type="monotone" dataKey="revenue" stroke="#2563EB" fill="url(#rGrad)" />
                </AreaChart>
              </ChartContainer>
            ) : (
              <div className="flex items-center justify-center h-[260px] text-slate-500 text-sm">No revenue data for this period</div>
            )}
          </div>
          ...
          <div className="bg-dark-card border border-dark-border rounded-2xl p-5">
            <div className="mb-5">
              <h2 className="text-slate-900 font-semibold text-lg">Daily Trip Count</h2>
            </div>
            {data?.daily?.length > 0 ? (
              ...
              <div className="flex items-center justify-center h-[200px] text-slate-500 text-sm">No data for this period</div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            <div className="bg-dark-card border border-dark-border rounded-2xl p-5">
              <h2 className="text-slate-900 font-semibold text-lg mb-4">Revenue by Ride Type</h2>
              {data?.byType?.length > 0 ? (
                <div className="space-y-3">
                  {data.byType.map((t, i) => {
                    const max = Math.max(...data.byType.map((x) => x.revenue), 1);
                    const pct = (t.revenue / max) * 100;
                    const colors = ['#2563EB', '#16A34A', '#D97706', '#DC2626', '#EA580C'];
                    return (
                      <div key={t.type}>
                        <div className="flex justify-between text-sm mb-1.5">
                          <span className="text-slate-600 capitalize">{t.type}</span>
                          <span className="text-slate-900 font-semibold">FC {Number(t.revenue).toLocaleString()}</span>
                        </div>
                        ...
                        <div className="text-slate-500 text-xs mt-0.5">{t.count} trips</div>
                      </div>
                    );
                  })}
                </div>
              ) : (
                <div className="flex items-center justify-center h-32 text-slate-500 text-sm">No type data</div>
              )}
            </div>

            <div className="bg-dark-card border border-dark-border rounded-2xl p-5">
              <h2 className="text-slate-900 font-semibold text-lg mb-4">Top Drivers</h2>
              {data?.topDrivers?.length > 0 ? (
                  ...
                  {data.topDrivers.slice(0, 8).map((d) => (
                    <div key={d.id} className="flex items-center gap-3 p-2.5 bg-dark-bg rounded-xl border border-dark-border">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-slate-700 text-sm font-medium truncate">{d.name}</div>
                        <div className="text-slate-500 text-xs">{d.trips} trips · ★ {d.rating.toFixed(1)}</div>
                      </div>
                ...
                <div className="flex items-center justify-center h-32 text-slate-500 text-sm">No driver data</div>
              )}
            </div>
          </div>
          ...
          {/* Hourly heatmap */}
          <div className="bg-dark-card border border-dark-border rounded-2xl p-5">
            <h2 className="text-slate-900 font-semibold text-lg mb-5">Hourly Trip Distribution</h2>
