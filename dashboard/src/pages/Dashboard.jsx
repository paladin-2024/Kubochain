...
function Ring({ value = 0, size = 80, stroke = 7, color = '#2563EB', label, sub }) {
...
// ─── Mini sparkline ──────────────────────────────────────────────────────────
function Sparkline({ data = [], dataKey = 'value', color = '#2563EB' }) {
...
function KpiCard({ label, value, sub, icon: Icon, color, sparkData, sparkKey, trend }) {
  const palette = {
    primary: { icon: 'text-primary bg-primary/10 border-primary/20', stroke: '#2563EB', glow: 'hover:border-primary/40' },
    success:  { icon: 'text-success bg-success/10 border-success/20', stroke: '#16A34A', glow: 'hover:border-success/40' },
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        {trend !== undefined && (
          <div className={`flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-lg ${
            ...
            {trend >= 0 ? <ChartUpIcon size={11} /> : <ChartDownIcon size={11} />}
            {Math.abs(trend)}%
          </div>
        )}
      </div>
      <div className="text-slate-500 text-xs font-medium uppercase tracking-wide mb-1">{label}</div>
      <div className="text-slate-900 text-2xl font-bold font-heading mb-1">{value ?? '—'}</div>
...
const EVENT_CONFIG = {
  completed:   { Icon: CheckmarkCircle01Icon, color: 'text-success',  bg: 'bg-success/10',  label: 'Trip completed' },
  cancelled:   { Icon: CancelCircleIcon,      color: 'text-danger',   bg: 'bg-danger/10',   label: 'Ride cancelled' },
  in_progress: { Icon: Navigation01Icon,      color: 'text-primary',  bg: 'bg-primary/10',  label: 'Trip in progress' },
  accepted:    { Icon: UserCheck01Icon,        color: 'text-success',  bg: 'bg-success/10',  label: 'Driver accepted' },
  arriving:    { Icon: MapPinpoint01Icon,      color: 'text-orange',   bg: 'bg-orange/10',   label: 'Driver arriving' },
  pending:     { Icon: Clock01Icon,            color: 'text-warning',  bg: 'bg-warning/10',  label: 'Ride requested' },
};
...
function RankBadge({ rank }) {
  const styles = [
    ...
    'bg-gray-400/20 text-slate-600 border-gray-400/30',
    'bg-orange/20 text-orange border-orange/30',
  ];
  return (
    <span className={`inline-flex items-center justify-center w-6 h-6 rounded-lg text-xs font-bold border flex-shrink-0 ${styles[rank - 1] || 'bg-dark-bg text-slate-500 border-dark-border'}`}>
      {rank}
    </span>
  );
}
...
export default function Dashboard() {
  ...
  const fetchData = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      setPendingDrivers(drivers.filter((d) => d.status === 'pending_approval'));
      setOnlineDrivers(
  ...
  return (
    <div className="p-6 space-y-6 min-h-full">
      ...
      {pendingDrivers.length > 0 && (
        <div
          onClick={() => navigate('/drivers?filter=pending')}
          className="flex items-center justify-between px-4 py-3 bg-warning/10 border border-warning/30 rounded-xl cursor-pointer hover:bg-warning/15 transition-colors"
        >
          <div className="flex items-center gap-3">
            <UserWarning01Icon size={18} className="text-warning flex-shrink-0" />
          ...
          <div className="flex items-center gap-1 text-warning text-xs font-medium">
            Review <ArrowRight01Icon size={13} />
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            {lastUpdated && (
              <span className="text-slate-500 text-xs">Updated {timeAgo(lastUpdated)}</span>
            )}
          </div>
          <h1 className="font-heading text-slate-900 text-2xl font-bold">{greeting}, Admin</h1>
          <p className="text-slate-500 text-sm mt-0.5">{dateStr} · KuboChain Operations Center</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          >
            <ChartUpIcon size={14} /> Reports
          </button>
          <button
            onClick={() => fetchData(true)}
            disabled={refreshing}
            className="flex items-center gap-2 px-4 py-2 bg-primary text-slate-800 rounded-xl text-sm font-medium hover:bg-primary/90 transition-all disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div className="xl:col-span-2 bg-dark-card border border-dark-border rounded-2xl p-5">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="font-heading text-slate-900 font-semibold text-lg">Revenue Trend</h2>
              <p className="text-slate-500 text-xs mt-0.5">Last 7 days — completed rides</p>
            </div>
            {dailyRevenue.length > 0 && (() => {
              ...
              return (
                <div className={`flex items-center gap-1 text-sm font-semibold px-3 py-1 rounded-xl ${
                  ...
                  {pct >= 0 ? <ChartUpIcon size={14} /> : <ChartDownIcon size={14} />}
          ...
          {dailyRevenue.length > 0 ? (
            ...
            <div className="flex items-center justify-center h-[200px] text-slate-500 text-sm">No revenue data yet</div>
          )}
          {dailyRevenue.length > 0 && (
            <div className="mt-3 flex justify-between gap-1">
              {dailyRevenue.map((d) => (
                <div key={d._id} className="flex-1 text-center">
                  <div className="text-slate-500 text-[10px] truncate">{d._id?.slice(5)}</div>
                  <div className="text-slate-600 text-xs font-semibold mt-0.5">{d.count || 0}</div>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="bg-dark-card border border-dark-border rounded-2xl p-5">
          <div className="mb-5">
            ...
            <p className="text-slate-500 text-xs mt-0.5">Real-time KPIs</p>
          </div>
          <div className="flex justify-around mb-6">
            <Ring value={completionRate} color="#16A34A" label="Completion" sub="All time" />
            <Ring value={driverUtil}     color="#2563EB" label="Driver Util" sub={`${stats?.onlineDrivers || 0} online`} />
          </div>
          ...
          <div className="mt-5 space-y-2">
            {[
              ...
              <div key={item.label} className="flex items-center justify-between text-xs">
                ...
                <span className="text-slate-900 font-semibold">{item.val.toLocaleString()}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div className="xl:col-span-2 bg-dark-card border border-dark-border rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="font-heading text-slate-900 font-semibold text-lg">Live Map</h2>
              <p className="text-slate-500 text-xs mt-0.5">{onlineDrivers.length} drivers tracked</p>
            </div>
        ...
        <div className="bg-dark-card border border-dark-border rounded-2xl p-5 flex flex-col">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-heading text-slate-900 font-semibold text-lg flex items-center gap-2">
              Active Rides
            ...
            {activeRides.length === 0 ? (
              ...
              <div key={ride._id} className="p-3 bg-dark-bg rounded-xl border border-dark-border hover:border-dark-border/80 transition-colors">
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex items-center gap-2">
                    <Avatar
                      name={ride.passenger ? `${ride.passenger.firstName} ${ride.passenger.lastName}` : ''}
                      size={28}
                    />
                ...
                <div className="flex items-center gap-1 text-xs text-slate-500 mb-1">
                  <MapPinpoint01Icon size={10} className="text-success flex-shrink-0" />
                  <span className="truncate">{ride.pickup?.address?.split(',')[0]}</span>
                </div>
                <div className="flex items-center gap-1 text-xs text-slate-500 mb-2">
                  <MapPinpoint01Icon size={10} className="text-danger flex-shrink-0" />
      ...
      {/* ── Activity Feed + Top Drivers + Command Actions ────────────────── */}
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">

        {/* Recent Activity */}
        <div className="bg-dark-card border border-dark-border rounded-2xl p-5">
          <h2 className="font-heading text-slate-900 font-semibold text-lg mb-4 flex items-center gap-2">
            <Activity01Icon size={16} className="text-primary" />
            Recent Activity
          </h2>
          <div className="space-y-3">
              ...
              return (
                <div key={ride._id} className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-slate-600 text-xs font-semibold">{cfg.label}</div>
                  ...
                  <div className="text-slate-500 text-[10px] flex-shrink-0 mt-0.5">
            ...
            {recentRides.length === 0 && (
              <div className="flex flex-col items-center py-8 text-slate-500 gap-2 text-sm">
                <Clock01Icon size={24} />
                No recent activity
              </div>
            )}
              ...
              return (
                <div key={d.id}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-slate-600 text-xs font-bold">FC {d.earnings.toLocaleString()}</span>
                  </div>
                  <div className="h-1.5 bg-dark-bg rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${pct}%`,
                        backgroundColor: ['#D97706','#94A3B8','#EA580C','#2563EB','#16A34A'][i],
                  ...
                  <div className="flex justify-between text-[10px] text-slate-500 mt-0.5">
                    ...
                    <span>{d.rating.toFixed(1)} avg rating</span>
                  </div>
                </div>
              );
            }) : (
        ...
        <div className="bg-dark-card border border-dark-border rounded-2xl p-5 space-y-4">
          <h2 className="font-heading text-slate-900 font-semibold text-lg flex items-center gap-2">
            <DashboardSpeed01Icon size={16} className="text-warning" />
            Command Center
          </h2>
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              { label: 'Passengers', value: stats?.totalPassengers?.toLocaleString() || 0, icon: UserGroupIcon, color: 'text-primary bg-primary/10' },
              { label: 'All Drivers', value: stats?.totalDrivers || 0, icon: UserCheck01Icon, color: 'text-success bg-success/10' },
              { label: 'Completed', value: stats?.completedRides?.toLocaleString() || 0, icon: CheckmarkCircle01Icon, color: 'text-success bg-success/10' },
              { label: 'Today', value: stats?.todayRides || 0, icon: Calendar01Icon, color: 'text-warning bg-warning/10' },
            ].map((s) => (
              <div key={s.label} className="bg-dark-bg border border-dark-border rounded-xl p-3">
                ...
                <div className="text-slate-900 text-lg font-bold leading-tight font-heading">{s.value}</div>
          ...
          <div className="space-y-1.5">
            <p className="text-slate-500 text-[10px] font-semibold uppercase tracking-widest mb-2">Quick Actions</p>
            {[
              { label: 'View all rides',    path: '/rides',         icon: Motorbike01Icon,   color: 'text-primary' },
              { label: 'Manage drivers',    path: '/drivers',       icon: UserCheck01Icon,    color: 'text-success' },
              { label: 'Send notification', path: '/notifications', icon: Notification01Icon, color: 'text-warning' },
              { label: 'Platform settings', path: '/settings',      icon: Shield01Icon,       color: 'text-orange'  },
              { label: 'Dispatch map',      path: '/dispatch',      icon: SignalFull01Icon,   color: 'text-primary' },
            ].map((a) => (
              <button
                key={a.path}
                onClick={() => navigate(a.path)}
                className="w-full flex items-center justify-between px-3 py-2.5 bg-dark-bg border border-dark-border rounded-xl text-sm text-slate-500 hover:border-primary/40 hover:text-slate-800 transition-all group"
              >
                <span className="flex items-center gap-2.5">
                  <a.icon size={14} className={a.color} />
                  {a.label}
                </span>
                <ArrowRight01Icon size={13} className="text-gray-700 group-hover:text-primary transition-colors" />
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="bg-dark-card border border-dark-border rounded-2xl p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-heading text-slate-900 font-semibold text-lg">Recent Rides</h2>
          <button
            onClick={() => navigate('/rides')}
